        ...
        GeometryReader{ gr in
            ScrollView(.vertical){
                VStack(alignment: .leading){
                    VStack(alignment: .leading){
                        PropertyView(imageNamme: "tram.circle"){
                            Text(poi.SIGUN_NM)
                        }
                        PropertyView(imageNamme: "location.circle"){
                            Text(poi.SIGUN_CD)
                        }
                        PropertyView(imageNamme: "fork.knife.circle"){
                            Text(poi.TASTFDPLC_TELNO)
                        }
                        PropertyView(imageNamme: "phone.circle"){
                            Button {
...
extension PoiItem {
    var location : CLLocationCoordinate2D{
        CLLocationCoordinate2D(latitude: Double(REFINE_WGS84_LAT)!, longitude: Double(REFINE_WGS84_LOGT)!)
    }
}
